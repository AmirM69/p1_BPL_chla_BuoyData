...
for i = 1:length(years)
    ...
    xlabel('Date')
     plot(df.Var1(year(df.Var1)==years(i)), df.pred_val_PC(year(df.Var1)==years(i)), 'color','b')
     plot(df.Var1(year(df.Var1)==years(i)), df.pred_val_ML(year(df.Var1)==years(i)), 'color','r')
...
legend({'chl^{PC}_{P}', 'chl^{ML}_{P}', 'chl_{M}'}, 'FontSize', 12)